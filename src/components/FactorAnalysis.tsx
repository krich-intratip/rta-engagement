...
export default function FactorAnalysis() {
    ...
    const radarData = groupStats.map((g) => ({
        label: g.groupName.replace("ในการทำงาน", "").replace("กับเพื่อนร่วมงาน", "").replace("และการบริหาร", "").replace("และค่าตอบแทน", "").replace("ผลและความก้าวหน้า", ""),
        ค่าเฉลี่ย: g.mean,
    }));
    ...
    if (filteredData.length === 0) {
        return (
            <div className="glass-card p-12 text-center">
                <p className="text-[var(--color-text-secondary)]">กรุณาโหลดข้อมูลก่อนเพื่อดูการวิเคราะห์ปัจจัย</p>
            </div>
        );
    }
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            <div className="glass-card p-5">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h2 className="text-base font-bold text-[var(--color-text)]">
                            ส่วนที่ 2 — ปัจจัยในงานที่ส่งผลต่อความสุขและความผูกพัน
                        </h2>
                        <p className="text-xs text-[var(--color-text-secondary)]">
                            29 ข้อ · 8 กลุ่มปัจจัย · {filteredData.length} คน
                        </p>
                    </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] text-center">
                        <p className="text-xs text-[var(--color-text-secondary)]">กลุ่มคะแนนสูงสุด</p>
                    ...
                    <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] text-center">
                        <p className="text-xs text-[var(--color-text-secondary)]">กลุ่มคะแนนต่ำสุด</p>
                    ...
                    <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] text-center">
                        <p className="text-xs text-[var(--color-text-secondary)]">ผู้ตอบแบบสำรวจ</p>
                        <p className="text-2xl font-extrabold text-[var(--color-primary)]">{filteredData.length}</p>
                        <p className="text-xs text-[var(--color-text-secondary)]">คน</p>
                    </div>
                </div>
            </div>

            {/* Radar Chart — Group Profile */}
            <div className="glass-card p-5">
                <h3 className="text-sm font-bold mb-3 text-[var(--color-text)]">โปรไฟล์คะแนนรายกลุ่มปัจจัย (Radar Chart)</h3>
                <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                        <RadarChart data={radarData}>
                            ...
                            />
                            <Radar dataKey="ค่าเฉลี่ย" stroke="#3B7DD8" fill="#3B7DD8" fillOpacity={0.25} strokeWidth={2} />
                        </RadarChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="glass-card p-5">
                <h3 className="text-sm font-bold mb-3 text-[var(--color-text)]">คะแนนเฉลี่ยรายกลุ่มปัจจัย</h3>
                <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                            data={groupStats.map((g) => ({ name: g.groupName, mean: g.mean, fill: GROUP_COLORS[g.groupName] ?? "#3B7DD8" }))}
                            layout="vertical"
                        ...
                        >
                            ...
                            <Tooltip
                                ...
                                formatter={(v: unknown) => [(v as number).toFixed(2), "คะแนนเฉลี่ย"]}
                            />
                            <Bar dataKey="mean" radius={[0, 4, 4, 0]} label={{ position: "right", fontSize: 10, formatter: (v: unknown) => (v as number).toFixed(2) }}>
                                {groupStats.map((g) => (
                                    <Cell key={g.groupName} fill={GROUP_COLORS[g.groupName] ?? "#3B7DD8"} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
            ...
            <div className="space-y-3">
                <h3 className="text-sm font-bold text-[var(--color-text)] px-1">รายละเอียดรายข้อ จำแนกตามกลุ่มปัจจัย</h3>
                {groupStats.map((group) => {
                    const expanded = isExpanded(group.groupName);
                    const color = GROUP_COLORS[group.groupName] ?? "#3B7DD8";
                        ...
                        <div key={group.groupName} className="glass-card overflow-hidden">
                            ...
                            {expanded && (
                                <div className="border-t border-[var(--color-border)] p-4 space-y-3">
                                    ...
                                    <div className="h-40">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={itemData} layout="vertical" margin={{ left: 0, right: 40 }}>
                                                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-chart-grid)" />
            ...
            <div className="glass-card p-5">
                <h3 className="text-sm font-bold mb-3 text-[var(--color-text)]">Top 5 ข้อที่ได้คะแนนสูงสุด / ต่ำสุด</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 mb-2">🏆 คะแนนสูงสุด 5 อันดับ</p>
                        <div className="space-y-2">
                            {[...itemMeans.map((m, i) => ({ m, i }))].filter((x) => x.m > 0).sort((a, b) => b.m - a.m).slice(0, 5).map(({ m, i }, rank) => (
                                <div key={i} className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 w-4">{rank + 1}</span>
                                    <span className="text-xs text-[var(--color-text)] flex-1 truncate" title={FACTOR_LABELS[i]}>{FACTOR_LABELS[i]}</span>
                    ...
                    <div>
                        <p className="text-xs font-semibold text-red-500 mb-2">⚠️ คะแนนต่ำสุด 5 อันดับ</p>
                        <div className="space-y-2">
                            {[...itemMeans.map((m, i) => ({ m, i }))].filter((x) => x.m > 0).sort((a, b) => a.m - b.m).slice(0, 5).map(({ m, i }, rank) => (
                                <div key={i} className="flex items-center gap-2">
